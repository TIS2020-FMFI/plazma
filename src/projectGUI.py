...
class ProjectGui:
    def __init__(self, main_gui):
        ...
        save_cal_button = tk.Button(project_frame, text="SAVE", bg='#bfc6db', fg='#323338',
                                    command=self.save_calibration, font=widget_button_font)
        ...
        load_cal_button = tk.Button(project_frame, text="LOAD", bg='#bfc6db', fg='#323338',
                                    command=self.load_calibration, font=widget_button_font)
        load_cal_button.grid(row=3, column=0, columnspan=5, padx=(215, 2), pady=(13, 7), sticky=tk.N + tk.W)

    def save(self, autosave=False):
        path = self.main_gui.program.file_manager.autosave_path
        if autosave and path:
            return True
        if not autosave or not path:
        ...
        self.main_gui.program.file_manager.save_project(path, name, description, autosave)
    ...
    def save_calibration(self):
        path = tk.filedialog.asksaveasfilename(initialfile="calibration", defaultextension=".txt")
        # path = tk.filedialog.askdirectory()
        if path == "":
            return False
        if self.main_gui.program.file_manager.save_calib(path):
            tk.messagebox.showinfo(title="Calibration", message="Calibration successfully saved!")
            return True
        tk.messagebox.showwarning(title="Calibration", message="No calibration to save!")
        return False
    ...
    def load_calibration(self):
        ...
        if path == "":
            return
        if self.main_gui.program.file_manager.load_calib(path):
            self.main_gui.info.change_calibration_label()
            tk.messagebox.showinfo(title="Calibration", message="Calibration successfully loaded!")
        else:
            tk.messagebox.showwarning(title="Calibration", message="Calibration failed to load!")
